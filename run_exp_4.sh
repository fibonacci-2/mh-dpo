...
# once a real per-skill ErrorRate_base(k) is measured.
#
# Usage: ./run_exp_4.sh
# Override defaults via env vars, e.g.: TRAIN_EPOCHS=2 ./run_exp_4.sh
# Everything (all 7 steps) is logged to a single file: logs/exp_4.log
set -euo pipefail
...
SKILL_WEIGHTS="${SKILL_WEIGHTS:-{\"SQ\":1.0,\"EV\":1.0,\"CR\":1.0}}"
...
CBTDP_TRAIN=data/cbtdp_dpo_train.csv
CBTDP_DIR=models/cbtdp-dpo-llama3.1-8b-lora

QUESTIONS=data/sampled_questions_shifaa.csv

RESULTS_DIR=results/ex4-cbtdp-dpo
